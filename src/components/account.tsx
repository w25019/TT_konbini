...
export const accountNav = [
  ...
  [MapPin, "お届け先管理", "/mypage/address"],
  [CreditCard, "支払い方法", "/mypage/payment"],
  [Ticket, "クーポン", "/mypage/coupons"],
  [Coins, "ポイント", "/mypage/points"],
  [User, "会員情報", "/mypage/profile"],
  [HelpCircle, "お問い合わせ", "/mypage/contact"],
] as const;
export function AccountNav() {
  return (
    <aside className="account-nav">
      <h3>♙　マイページ</h3>
      {accountNav.map(([I, t, h]) => (
        <Link href={h} key={t}>
          <I />
          {t}
          <ChevronRight />
...
export function AccountGuard({ children }: { children: React.ReactNode }) {
  ...
  return s.hydrated && s.isAuthenticated ? (
    <>{children}</>
  ) : (
